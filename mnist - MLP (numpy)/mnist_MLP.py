...
class NeuralNetwork(object):
	def __init__(self, input_dim, hidden_dim, output_dim, lr=0.025):
		...
		self.W1 = np.random.uniform(low = -1.0, high = 1.0, size = (input_dim, hidden_dim))
		self.b1 = np.zeros((hidden_dim,))

		# Output Layer Weights
		self.W2 = np.random.uniform(low = -1.0, high = 1.0, size = (hidden_dim, output_dim))
...
for i in range(epoch):
	...
	for j in range(steps):
		nn.train(x_train[mini_batch*j:mini_batch*j+mini_batch], y_train[mini_batch*j:mini_batch*j+mini_batch])
	print(nn.cross_entropy(nn.forward(x_train),y_train))
